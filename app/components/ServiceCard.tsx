    ...
    <div className="border border-border rounded-lg p-6 bg-background hover:shadow-lg transition-shadow">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-foreground/5 rounded-lg p-3">
          ...
          <div className="text-2xl font-bold text-foreground">{service.uptime24h.toFixed(2)}%</div>
        </div>
        <div className="bg-foreground/5 rounded-lg p-3">
          <div className="text-foreground/60 text-xs mb-1">7d Uptime</div>
          <div className="text-2xl font-bold text-foreground">{service.uptime7d.toFixed(2)}%</div>
        </div>
        <div className="bg-foreground/5 rounded-lg p-3">
          <div className="text-foreground/60 text-xs mb-1">30d Uptime</div>
          <div className="text-2xl font-bold text-foreground">{service.uptime30d.toFixed(2)}%</div>
        </div>
        {service.currentLatency !== undefined && (
          <div className="bg-foreground/5 rounded-lg p-3">
            <div className="text-foreground/60 text-xs mb-1">Current Latency</div>
            <div className="text-2xl font-bold text-foreground">{service.currentLatency}ms</div>
          </div>
        )}
      </div>
