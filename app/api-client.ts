 ...
 */
export async function fetchServiceHistory(serviceName: string): Promise<ServiceHistoryResponse> {
  const response = await fetch(`/api/service/${encodeURIComponent(serviceName)}/history`);
  
  if (!response.ok) {
    throw new Error(`Failed to fetch history for ${serviceName}: ${response.statusText}`);
  }
